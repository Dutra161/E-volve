...
def detailed_ploter(_target,rtt_path, stc_path, matches, rtt_dismatches, stc_dismatches, cutoff, _type):
    #create a pml file for the full structure superposition with the matchs in highlights
    folder = OSfunct.create_dir(_target+"/Plots",rtt_path,stc_path,_type)
    #define ne file name
    pmlname = _target+"/Plots/"+folder+"/c_scale" + rtt_path[rtt_path.rfind("/")+1:rtt_path.rfind("_")] + "_x_" + stc_path[stc_path.rfind("/")+1:-4] + ".pml"
    pml = open(pmlname,'w')
